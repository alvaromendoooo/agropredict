...
try {
return { result: await dataService.variedades(cultivo) };
} catch {
return { result: [] };
}
},
...
try {
return { result: await dataService.variedadUmbrales(nombre) };
} catch (err) {
...
if (!nombre) return fail(400, { message: 'A variety name is required' });
try {
return { result: await dataService.variedadHorasFrio(nombre) };
} catch (err) {
const outcome = errorOutcome(err);
return fail(outcome.status, { message: outcome.message });
}
...
try {
return { result: await dataService.modeloVariedades(codigo) };
} catch (err) {
const outcome = errorOutcome(err);
return fail(outcome.status, { message: outcome.message });
}
