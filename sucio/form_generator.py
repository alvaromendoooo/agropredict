...
class InformeService():
    ...
    @staticmethod
    def color_porcentaje_riesgo(
        datos_tabla : list[list[str]],
        tabla
    ):
        # Determino la columna del porcentaje
        col_porcentaje = -1

        for i, fila in enumerate(datos_tabla[1:], start = 1): # Empieza desde la primera fila de datos
            try:
                valor_str = fila[col_porcentaje].replace('%', '')
                valor = float(valor_str)

                if valor >= 70:
                    color_fondo = colors.HexColor("#FFCCCC")  # Rojo claro para alto riesgo
                elif valor >= 40:
                    color_fondo = colors.HexColor("#FFFFCC")  # Amarillo claro para riesgo medio
                elif valor >= 10:
                    color_fondo = colors.HexColor("#CCFFCC")  # Verde claro para riesgo bajo

                tabla.setStyle(TableStyle([
                    ('BACKGROUND', (col_porcentaje, i), (col_porcentaje, i), color_fondo),
                ]))
            except: 
                pass
    ...
    def crear_informe(predicciones: dict):
        ...
        try:
            ...
            if datos_variedades: # Quiero que rellene la tabla con datos de variedades predecidas
                ...
                datos_tabla = [["Fecha", "Temp. Min (ºC)", "Variedad", "Nivel de Riesgo", "Riesgo %"]]
                for p in datos_variedades:
                    ...
                    datos_tabla.append(
                        [
                            ...
                            p.get('variedad', "-"),
                            nivel,
                            f"{p.get('porcentaje_riesgo', 0)}%"
                        ]
                    )
            elif datos_localidades:
                print(f"Datos localidades : {datos_localidades}")
                # Modifico la cabecera general
                datos_tabla = [["Fecha", "Localidad", "Provincia", "Temp. Mín", "Temp. Max", "Nivel de Riesgo", "Riesgo %"]]
            ...
            if datos_variedades:
                ...
                col_widths = [0.9 * inch, 1.1 * inch, 0.9 * inch, 0.9 * inch, 0.9 * inch, 1.0 * inch, 0.8 * inch]
            else:
            ...
            InformeService.color_porcentaje_riesgo(
